...
import { Blinds, Loader2, TrendingUp } from "lucide-react";
...
interface Entry {
  ...
  region: string; address: string; deliveryDate: string | null; status: string;
  curtainCount: number; curtainTotal: number;
...
// Status → colour classes for the pill/select.
...
export default function CurtainOrdersClient() {
  ...
  // Persist one field and recompute the row locally (difference / totals).
      ...
      {summary && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-5">
          <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-3">
            ...
            <div className="text-lg font-black text-green-700 dark:text-green-300">{fmt(totalProfit)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
            <div className="text-xs font-bold text-gray-500 mb-1">{t("coSumOur")}</div>
            <div className="text-lg font-black text-gray-900 dark:text-gray-100">{fmt(summary.totalOur)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
            <div className="text-xs font-bold text-gray-500 mb-1">{t("coSumOutside")}</div>
            <div className="text-lg font-black text-gray-900 dark:text-gray-100">{fmt(summary.totalOutside)}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
            <div className="text-xs font-bold text-gray-500 mb-1">{t("coSumCount")}</div>
            <div className="text-lg font-black text-gray-900 dark:text-gray-100">{summary.count}</div>
          </div>
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3">
            <div className="text-xs font-bold text-gray-500 mb-1">{t("coSumCurtains")}</div>
            <div className="text-lg font-black text-gray-900 dark:text-gray-100">{summary.totalCurtains}</div>
          </div>
        </div>
      )}

      <datalist id="co-manufacturers">
              ...
              {entries.map((e, i) => {
                ...
                return (
                  <tr key={e.id} className="border-b border-gray-100 dark:border-gray-700/50 hover:bg-gray-50/60 dark:hover:bg-gray-700/30">
                    ...
                    <td className="p-2 text-center whitespace-nowrap text-gray-700 dark:text-gray-300">{fmtDate(e.deliveryDate)}</td>
                    <td className="p-2 whitespace-nowrap text-gray-700 dark:text-gray-300" dir="ltr">{e.phone ? `${e.phoneCode || ""}${e.phone}` : "—"}</td>
                    <td className="p-2 text-gray-700 dark:text-gray-300 max-w-[180px] truncate" title={e.region || e.address}>{e.region || e.address || "—"}</td>
                    <td className="p-2 text-center text-gray-700 dark:text-gray-300">{e.curtainCount || "—"}</td>
                    <td className="p-2">
                      <input
                        value={val(e, "advanceBillNo") ?? ""}
                        onChange={(ev) => setDraftVal(e.id, "advanceBillNo", ev.target.value)}
                        onBlur={(ev) => { clearDraft(e.id, "advanceBillNo"); if (ev.target.value !== e.advanceBillNo) save(e.id, { advanceBillNo: ev.target.value }); }}
                        placeholder="SW-000"
                        className="w-24 px-2 py-1 rounded border border-gray-200 dark:border-gray-600 bg-transparent text-center text-xs outline-none focus:border-[#6f7e62]" />
                    </td>
                    <td className="p-2">
                      <input
                        type="number" step="0.001" min="0"
                        value={val(e, "ourPrice") ?? ""}
                        onChange={(ev) => setDraftVal(e.id, "ourPrice", ev.target.value)}
                        onBlur={(ev) => { clearDraft(e.id, "ourPrice"); const n = ev.target.value === "" ? null : Number(ev.target.value); if (n !== (e.ourPriceOverridden ? e.ourPrice : null)) save(e.id, { ourPrice: n }); }}
                        placeholder={fmt(e.curtainTotal)}
                        title={e.ourPriceOverridden ? "" : t("coDefault")}
                        className={`w-24 px-2 py-1 rounded border bg-transparent text-center text-xs outline-none focus:border-[#6f7e62] ${e.ourPriceOverridden ? "border-gray-200 dark:border-gray-600" : "border-dashed border-gray-300 dark:border-gray-600 text-gray-500"}`} />
                    </td>
                    <td className="p-2">
                      <input
                        type="number" step="0.001" min="0"
                        value={val(e, "outsidePrice") ?? ""}
                        onChange={(ev) => setDraftVal(e.id, "outsidePrice", ev.target.value)}
                    ...
                    <td className="p-2">
                      <input
                        list="co-manufacturers"
                        value={val(e, "manufacturer") ?? ""}
                        onChange={(ev) => setDraftVal(e.id, "manufacturer", ev.target.value)}
                        onBlur={(ev) => { clearDraft(e.id, "manufacturer"); if (ev.target.value !== e.manufacturer) save(e.id, { manufacturer: ev.target.value }); }}
                        placeholder={t("coManufacturerPlaceholder")}
                        className="w-32 px-2 py-1 rounded border border-gray-200 dark:border-gray-600 bg-transparent text-xs outline-none focus:border-[#6f7e62]" />
                    </td>
                    <td className="p-2 text-center">
                      <select
                        value={e.workStatus}
                        onChange={(ev) => save(e.id, { workStatus: ev.target.value })}
