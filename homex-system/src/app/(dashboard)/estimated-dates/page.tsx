...
export default function EstimatedDatesPage() {
  ...
  const confirm = async (id: string) => {
    const date = dates[id];
    if (!date) { toast.error("أدخل التاريخ الصحيح أولاً"); return; }
    setBusy(id);
    try {
      ...
      toast.success("تم تأكيد التاريخ");
  ...
  return (
    <div className="max-w-4xl">
      <div className="mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2"><CalendarClock className="w-6 h-6" /> تأكيد التواريخ التقديرية</h1>
        <p className="text-sm text-gray-500 mt-1">
          هذه الطلبات أخذت تاريخاً <b>تقديرياً</b> من الاستيراد (من الصفوف المجاورة) لترتيبها فقط. أدخل التاريخ الصحيح واضغط «تأكيد» ليصبح نهائياً وتختفي البشارة.
        </p>
      </div>

      {loading ? (
        <div className="flex items-center gap-2 text-gray-500 text-sm"><Loader2 className="w-4 h-4 animate-spin" /> جارٍ التحميل...</div>
      ) : rows.length === 0 ? (
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-12 text-center">
          <Check className="w-12 h-12 text-emerald-500 mx-auto mb-3" />
          <p className="text-gray-600 dark:text-gray-300 font-semibold">لا توجد تواريخ تقديرية — كل التواريخ مؤكَّدة ✓</p>
        </div>
      ) : (
        <>
          <p className="text-sm text-gray-400 mb-3"><span className="font-mono-en font-bold">{rows.length}</span> طلب بحاجة لتأكيد التاريخ</p>
              ...
              <div key={r.id} className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 flex flex-col sm:flex-row sm:items-center gap-3">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {r.originalNumber && <span className="text-[10px] font-mono-en px-1 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-gray-500">مرجع: {r.originalNumber}</span>}
                    <span className="rounded px-1 py-0.5 text-[10px] font-bold bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300">تقديري</span>
                  </div>
                  <p className="text-sm font-bold mt-0.5 truncate">{r.customerName}</p>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <input type="time" value={times[r.id] || ""} onChange={(e) => setTimes({ ...times, [r.id]: e.target.value })}
                    className="field h-10 w-24 font-mono-en text-sm" aria-label="الوقت" />
                  <button onClick={() => confirm(r.id)} disabled={busy === r.id || !dates[r.id]}
                    ...
                    {busy === r.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} تأكيد
                  </button>
                </div>
              </div>
            ))}
          ...
          <p className="text-xs text-gray-400 mt-4 flex items-center gap-1">
            <ExternalLink className="w-3 h-3" /> اضغط رقم الطلب لفتحه كاملاً لو تحتاج تعديل تفاصيل أخرى.
          </p>
        </>
      )}
    </div>
