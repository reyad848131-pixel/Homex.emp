  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        {/* Settings access — owner-only (Riyad / Salim) + designated employees */}
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded p-6">
          <h2 className="text-base font-bold mb-1">{t("saSetting")}</h2>
          <p className="text-xs text-gray-400 mb-4">{t("saHint")}</p>
          {(() => {
            const OWNER_CIVIL_IDS = ["2016", "1389"];
            let ids: string[] = [];
            try { const a = JSON.parse(settings.settings_access_editors || "[]"); if (Array.isArray(a)) ids = a.filter((x) => typeof x === "string"); } catch { /* ignore */ }
            const toggle = (id: string) => {
              const next = ids.includes(id) ? ids.filter((x) => x !== id) : [...ids, id];
              update("settings_access_editors", JSON.stringify(next));
            };
            if (employees.length === 0) return <p className="text-sm text-gray-400">{t("peNoOne")}</p>;
            return (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-w-xl">
