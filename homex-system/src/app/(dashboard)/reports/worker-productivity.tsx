...
function ranges() {
  ...
  return {
    month: { from: ym(y, m, 1), to: ym(y, m, new Date(y, m + 1, 0).getDate()), label: "هذا الشهر" },
    last: { from: ym(lastMonth.getFullYear(), lastMonth.getMonth(), 1), to: ym(lastMonth.getFullYear(), lastMonth.getMonth(), new Date(lastMonth.getFullYear(), lastMonth.getMonth() + 1, 0).getDate()), label: "الشهر الماضي" },
    year: { from: `${y}-01-01`, to: `${y}-12-31`, label: "هذه السنة" },
  };
}
    ...
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5 mb-6">
      <div className="flex items-center justify-between flex-wrap gap-3 mb-5">
        <h2 className="text-sm font-bold text-gray-600 dark:text-gray-300 flex items-center gap-2">
          <HardHat className="w-4 h-4" /> إنتاجية العمّال
        </h2>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400">
          ...
          <p className="text-sm font-semibold">لا توجد مراحل منجزة بعد في هذه الفترة.</p>
          <p className="text-xs mt-1">عيّن العمّال على مراحل الأصناف من «إدارة الأعمال» وعلّمها كمنجزة لتظهر هنا.</p>
        </div>
      ) : (
        <>
          {/* KPI row */}
          <div className="grid grid-cols-3 gap-3 mb-5">
            <div className="rounded-lg border border-gray-100 dark:border-gray-700 p-3">
              <div className="text-[11px] text-gray-500 font-bold flex items-center gap-1 mb-1"><CheckCircle2 className="w-3.5 h-3.5" /> مراحل منجزة</div>
              <div className="text-2xl font-black font-mono-en">{data!.totals.totalCompleted}</div>
            </div>
            <div className="rounded-lg border border-gray-100 dark:border-gray-700 p-3">
              <div className="text-[11px] text-gray-500 font-bold flex items-center gap-1 mb-1"><Users className="w-3.5 h-3.5" /> عمّال نشطون</div>
              <div className="text-2xl font-black font-mono-en">{data!.totals.activeWorkers}</div>
            </div>
            <div className="rounded-lg border border-gray-100 dark:border-gray-700 p-3">
              <div className="text-[11px] text-gray-500 font-bold flex items-center gap-1 mb-1"><Clock className="w-3.5 h-3.5" /> في الوقت</div>
          ...
          {data!.star && data!.star.completed > 0 && (
            <div className="flex items-center gap-3 p-3.5 mb-5 rounded-xl border border-amber-200 dark:border-amber-800/60 bg-gradient-to-l from-amber-50 to-transparent dark:from-amber-900/15">
              ...
              <div className="min-w-0">
                <div className="font-black flex items-center gap-2"><Avatar color={data!.star.color} name={data!.star.name} size={24} /> {data!.star.name} — نجم الفترة</div>
                <div className="text-xs text-gray-500 mt-0.5">
                  <span className="font-mono-en font-bold">{data!.star.completed}</span> مرحلة منجزة
                  {data!.star.onTimeRate != null && <> · <span className="font-mono-en font-bold">{data!.star.onTimeRate}%</span> في الوقت</>}
                </div>
              </div>
            </div>
          )}
            ...
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="text-[11px] text-gray-400 font-bold">
                  ...
                  <th className="text-start pb-2">العامل</th>
                  <th className="text-start pb-2 min-w-[140px]">الإنتاج (مراحل منجزة)</th>
                  <th className="text-start pb-2">في الوقت</th>
                  <th className="text-start pb-2">قيد التنفيذ</th>
                  <th className="text-start pb-2">آخر نشاط</th>
                </tr>
              </thead>
              <tbody>
                {data!.rows.filter((r) => r.completed > 0 || r.currentLoad > 0).map((r, i) => (
                  <tr key={r.id} className="border-t border-gray-100 dark:border-gray-700">
                    ...
                    <td className="py-2.5"><div className="flex items-center gap-2 font-bold"><Avatar color={r.color} name={r.name} size={28} /> {r.name}{!r.isActive && <span className="text-[10px] text-gray-400 font-normal">(معطّل)</span>}</div></td>
                    <td className="py-2.5">
                    ...
                    <td className="py-2.5 text-gray-500 text-xs">{timeAgo(r.lastDone)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
