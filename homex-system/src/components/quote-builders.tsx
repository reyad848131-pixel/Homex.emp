...
// every count field matches. Clamps to [min, max] on each press.
export function StepperRow({
  label, hint, value, onChange, min, max, lineTotal,
}: {
  ...
  const btn =
    "w-9 h-9 rounded-full border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 font-bold text-lg leading-none flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors disabled:opacity-30";
  return (
    <div className={cn("flex items-center gap-3 rounded-lg border px-3 py-2.5 transition-colors",
      active
      ...
      <div className="flex-1 min-w-0">
        ...
        {hint != null && <p className="text-xs text-gray-400 font-mono-en">{hint}</p>}
      </div>
      <div className="flex items-center gap-1.5 flex-shrink-0">
        <button type="button" aria-label="+" className={btn}
          onClick={() => onChange(max != null ? Math.min(max, value + 1) : value + 1)}
          disabled={max != null && value >= max}>+</button>
        <span className="w-8 text-center font-mono-en font-black text-gray-900 dark:text-gray-50">{value}</span>
        <button type="button" aria-label="−" className={btn}
      ...
      {lineTotal != null && (
        <div className="w-16 text-left flex-shrink-0">
          {active && <span className="text-xs font-bold text-gray-600 dark:text-gray-300 font-mono-en">{lineTotal}</span>}
        </div>
      )}
    </div>
  );
...
function KitchenBuilder({ config, governorate, wilayat, onUpdate, initial }: { config: any; governorate: string; wilayat: string; onUpdate: BuilderUpdate; initial?: BuilderInitial }) {
  ...
  // Accessory quantities keyed by id. Back-compat: older items stored `accIds`
  // (a plain selection) — treat each as a count of 1.
  const [accCounts, setAccCounts] = useState<Record<string, number>>(
    initial?.accCounts ??
    (Array.isArray(initial?.accIds) ? Object.fromEntries((initial.accIds as string[]).map((id) => [id, 1])) : {})
  );
  ...
  const accCount = (id: string) => Math.max(0, Math.round(accCounts[id] || 0));
  ...
  useEffect(() => {
    ...
    const names = accessories.filter((a) => accCount(a.id) > 0).map((a) => {
...
function CabinetBuilder({ config, onUpdate, initial }: { config: any; onUpdate: BuilderUpdate; initial?: BuilderInitial }) {
  ...
  return (
    <div className="space-y-4">
            ...
            {GLASS_OPTIONS.map((o) => {
              ...
              return (
                <StepperRow key={o.key}
                  ...
                  min={0}
                  lineTotal={c > 0 ? (c * o.price).toFixed(3) : undefined}
                />
              );
            })}
      ...
      <StepperRow
        label={t("ledLighting")}
        hint={`${config.led || 25} ${t("omr")}`}
        value={leds}
        onChange={setLeds}
        min={0}
        lineTotal={leds > 0 ? (leds * (config.led || 25)).toFixed(3) : undefined}
      />
      <div>
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {lighting && (
          <div className="mt-2">
            <StepperRow
              label={t("lightPointsCount")}
              hint={`${LIGHT_PRICE.toFixed(3)} ${t("omr")}`}
              value={lightCount}
              onChange={setLightCount}
              min={1}
              lineTotal={lightSurcharge.toFixed(3)}
            />
          </div>
        )}
      </div>
...
function SofaBuilder({ onUpdate, initial }: { config: any; onUpdate: BuilderUpdate; initial?: BuilderInitial }) {
  ...
  const [manualPrice, setManualPrice] = useState<number>(initial?.manualPrice ?? 80);
  const [note, setNote] = useState<string>(initial?.note ?? "");
  const [rateOverride, setRateOverride] = useState<number | null>(initial?.rateOverride ?? null);
  const [priceOverride, setPriceOverride] = useState<number | null>(initial?.priceOverride ?? null);
  ...
  return (
    <div className="space-y-4">
      {/* Pricing mode: per-piece (structured) OR a free manual price. */}
      <div className="flex gap-2">
        {([["pieces", t("sofaModePieces")], ["manual", t("sofaModeManual")]] as const).map(([k, l]) => (
          <button key={k} type="button" onClick={() => setMode(k)}
            className={cn("flex-1 py-2 rounded-lg text-sm font-bold border transition-colors",
              mode === k ? "bg-teal-600 text-white border-teal-600" : "bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300")}>
            {l}
          </button>
        ))}
      </div>
      ...
      {mode === "pieces" ? (
        ...
        <div>
          <label className="block text-sm font-semibold text-gray-600 mb-1.5">{t("price")} ({t("omr")})</label>
          <NumStepper value={manualPrice} onChange={setManualPrice} min={0} step={5} className="w-full" />
        </div>
      )}
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {lighting && (
          <div className="mt-3">
            <StepperRow
              label={t("lightingsCount")}
              hint={`${config.lighting || 20} ${t("omr")}`}
              value={lightCount}
              onChange={setLightCount}
              min={1}
              lineTotal={(lightCount * (config.lighting || 20)).toFixed(3)}
            />
          </div>
        )}
      </div>
...
function GenericBuilder({ cat, onUpdate, initial }: { cat: Category; onUpdate: BuilderUpdate; initial?: BuilderInitial }) {
  ...
  const defaultRate =
    cat.pricingType === "per_sqm"
      ? (genConfig.pricePerSqm || cat.basePrice || 50)
      : cat.pricingType === "per_meter"
